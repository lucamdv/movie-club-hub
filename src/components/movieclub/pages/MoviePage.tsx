            ...
            <div>
              ...
              {existingRating && (
                <p style={{ fontSize: 12, color: C.gold, marginTop: 2 }}>
                  Sua nota atual: {Number(existingRating.rating).toFixed(1)} ★
                </p>
              )}
            </div>
          </div>
            ...
            {[1, 2, 3, 4, 5].map((s) => (
              <button
                ...
                style={{
                  ...
                  transform: localRating >= s ? "scale(1.2)" : "scale(1)",
                  minHeight: "unset",
                  minWidth: "unset",
                }}
              >
          ...
          <div style={{ display: "flex", gap: 10, marginTop: 16 }}>
            ...
            <button
              ...
              style={{
                ...
                padding: "14px",
                borderRadius: 12,
                background: localRating ? `linear-gradient(135deg, ${C.goldDim}, ${C.gold})` : C.bgCard,
                color: localRating ? C.bgDeep : C.textDim,
                fontSize: 15,
                fontWeight: 700,
                fontFamily: "'Outfit', sans-serif",
                border: "none",
        ...
        <div style={{ position: "relative", height: "min(260px, 38dvh)", minHeight: 200, overflow: "hidden" }}>
          ...
          <div
            style={{
              position: "absolute",
              inset: 0,
              background: `linear-gradient(to bottom, rgba(15,25,35,0.55) 0%, transparent 35%, ${C.bg} 100%)`,
            }}
          />
          ...
          <button
            onClick={() => {
              if (navigator.share) navigator.share({ title: m.title, url: window.location.href });
            }}
            style={{
              position: "absolute",
              top: "calc(var(--safe-top, 0px) + 12px)",
        ...
        <div style={{ margin: "-70px 14px 0", position: "relative", zIndex: 2 }}>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 20 }}>
            ...
            >
              <StarIcon
                size={17}
                fill={existingRating ? C.gold : "none"}
                stroke={existingRating ? C.gold : C.textMuted}
              />
          ...
          >
            ...
            {(m.plot || m.overview)?.length > 150 && (
              ...
              >
                ...
                <ChevronDown
                  size={14}
                  style={{ transform: showFullOverview ? "rotate(180deg)" : "none", transition: "transform 0.2s" }}
                />
              </button>
            )}
          </div>
              ...
              >
                {m.similar.slice(0, 8).map((s) => (
                  <div
                    ...
                    style={{ flexShrink: 0, cursor: "pointer", width: 92, scrollSnapAlign: "start" }}
                  >
                    <div
                      style={{
                        height: 135,
    ...
    <div style={{ paddingTop: 60, paddingBottom: 60, minHeight: "100dvh", overflowX: "hidden" }}>
      ...
      <div style={{ maxWidth: 1100, margin: "-200px auto 0", padding: "0 32px", position: "relative", zIndex: 1 }}>
          ...
          <div style={{ flex: 1, paddingBottom: 6 }}>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 10 }}>
              {(m.genres?.length ? m.genres : [m.genre]).slice(0, 4).map((g) => (
                <Badge key={g} color="rgba(201,168,76,0.10)" textColor={C.goldDim}>
                  {g}
                </Badge>
              ))}
              {m.rated && (
            ...
            {m.tagline && (
              <p style={{ color: C.gold, fontSize: 13, fontStyle: "italic", marginBottom: 14 }}>"{m.tagline}"</p>
            )}
            <div style={{ display: "flex", gap: 20, marginBottom: 14, flexWrap: "wrap" }}>
              {m.director && (
                <div>
                  <span style={{ fontSize: 10, color: C.textDim, display: "block" }}>Direção</span>
                  <span style={{ fontSize: 13, color: C.textMuted }}>{m.director}</span>
                </div>
              )}
              {m.writer && (
                <div>
                  <span style={{ fontSize: 10, color: C.textDim, display: "block" }}>Roteiro</span>
                  <span style={{ fontSize: 13, color: C.textMuted }}>{m.writer.split(",")[0]}</span>
                </div>
              )}
            </div>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              ...
              <Btn variant="ghost" onClick={() => setShowRatingSheet(true)}>
                <StarIcon
                  size={14}
                  fill={existingRating ? C.gold : "none"}
                  stroke={existingRating ? C.gold : "currentColor"}
                />
                {existingRating ? `${Number(existingRating.rating).toFixed(1)} ★` : "Avaliar"}
              </Btn>
              {m.trailer && (
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 290px", gap: 28 }}>
          ...
          <div>
            {[
              { title: "Similares", data: m.similar },
              { title: "Recomendados", data: m.recommendations },
            ].map(({ title, data }) =>
              data?.length > 0 ? (
